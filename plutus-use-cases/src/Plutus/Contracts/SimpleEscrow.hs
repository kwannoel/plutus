  ...
  where
    redeem params = do
      ...
      let value = foldMap (Tx.txOutValue . Tx.txOutTxOut) unspentOutputs
          beforeDeadlineRange = Interval.to (Haskell.pred $ deadline params)
          tx = Typed.collectFromScript unspentOutputs Redeem
                      <> Constraints.mustValidateIn beforeDeadlineRange
...
refundEp = mapError (review _EscrowError) $ endpoint @"refund" >>= refund
  where
    refund params = do
      ...
      let afterDeadlineRange = Interval.from (Haskell.succ $ deadline params)
          tx = Typed.collectFromScript unspentOutputs Refund
                  <> Constraints.mustValidateIn afterDeadlineRange
